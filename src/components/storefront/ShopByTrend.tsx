  ...
  return (
    <section className="section-padding-sm bg-cream">
      <div className="container-premium">
        <h2 className="font-serif text-3xl sm:text-4xl text-navy text-center mb-10 sm:mb-14">Shop By Trend</h2>
        <div className="grid md:grid-cols-2 gap-4 sm:gap-6">
          {trends.map(({ title, subtitle, href, image, align }) => (
            <Link
              key={title}
              href={href}
              className="group relative aspect-[4/5] sm:aspect-[16/10] md:aspect-[4/5] overflow-hidden rounded-2xl sm:rounded-3xl"
            >
              <Image
                src={image}
                alt={title}
              ...
              <div
                className={`absolute bottom-0 left-0 right-0 p-6 sm:p-8 text-cream ${align === "right" ? "md:text-right" : ""}`}
              >
                <h3 className="font-serif text-2xl sm:text-3xl lg:text-4xl mb-2 leading-tight">{title}</h3>
